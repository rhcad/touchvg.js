...
public:
    ...
    /*!
        ...
        \param originW Original line width, >0: in 0.01mm unit, 0: 1px, <0: in point unit.
     */
    virtual void setPen(int argb, float width, int style, float phase, float originW) = 0;
    ...
    //! Ready to add a sub-path to the current path at the start point.
    virtual void moveTo(float x, float y) = 0;
    
    //! Add a line segment to the current sub-path.
    virtual void lineTo(float x, float y) = 0;
    
    //! Add a cubic bezier segment to the current sub-path.
    virtual void bezierTo(float c1x, float c1y, float c2x, float c2y, float x, float y) = 0;
    
    //! Add a quadratic bezier segment to the current sub-path.
    virtual void quadTo(float cpx, float cpy, float x, float y) = 0;
    
    //! Close the current sub-path of the path added by beginPath().
    ...
    virtual void drawHandle(float x, float y, int type, float angle) = 0;
     ...
     */
    virtual void drawTextAt(const char* text, float x, float y, float h, int align, float angle) = 0;
